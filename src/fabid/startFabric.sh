...
pushd ../test-network
./network.sh down
./network.sh up createChannel -c fabidchannel -ca -s couchdb
./network.sh deployCC -c fabidchannel -ccn fabid -ccv 1 -cci initLedger -ccl ${CC_SRC_LANGUAGE} -ccp ${CC_SRC_PATH}
popd
